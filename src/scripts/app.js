...
function handleChartZoom(minDate, maxDate) {
	...
	const { visaRecords, mastercardRecords, ecbRecords } = ChartManager.getVisibleRecordsByProvider(currentVisaRecords, currentMcRecords, currentEcbRecords, minDate, maxDate);
...
async function loadCurrencyPair() {
	...
	try {
		...
		const result = await DataManager.fetchRates(/** @type {CurrencyCode} */ (fromCurr), /** @type {CurrencyCode} */ (toCurr), range, {
		...
		if (ChartManager.isChartInitialized()) {
			...
			await ChartManager.initChart("chart", result.visaRecords, result.mastercardRecords, result.ecbRecords, fromCurr, toCurr);
		}
